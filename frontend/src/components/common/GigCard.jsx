...
const GigCard = ({ data, isUrgent = false, onApply }) => {
  const { id, title, match, time, dist, price, description, client, videoUrl } = data;
    ...
    <div className={`bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all flex flex-col justify-between group border border-slate-100 ${isUrgent ? 'md:col-span-2 relative border-l-4 border-l-blue-600' : ''}`}>
      <div className={isUrgent ? 'flex flex-col md:flex-row gap-6 items-start relative z-10' : ''}>
        <div className="flex-1 w-full">
          ...
          {videoUrl ? (
            <div className="mb-4 rounded-xl overflow-hidden bg-slate-50 border border-slate-100">
              <video 
                src={videoUrl} 
                controls 
            ...
            <div className="mb-4 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center h-32 text-slate-400 text-xs font-bold uppercase tracking-widest">
              No Video Attached
            </div>
          )}

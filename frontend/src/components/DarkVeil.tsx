...
void mainImage(out vec4 fragColor,in vec2 fragCoord){
    ...
    if (uLightMode > 0.5) {
        // Calculate greyscale intensity of the pattern
        float intensity = dot(fragColor.rgb, vec3(0.299, 0.587, 0.114));
        
        // Use the pure brand blue
        fragColor.rgb = vec3(0.145, 0.388, 0.921); 
        
        // Map the intensity directly to the alpha channel instead of mixing with white
        fragColor.a = intensity * 1.5;
    } else {
        fragColor.a = 1.0;
    }
}
...
void main(){
    ...
    gl_FragColor=vec4(clamp(col.rgb,0.0,1.0), clamp(col.a, 0.0, 1.0) * uOpacity);
}
`;

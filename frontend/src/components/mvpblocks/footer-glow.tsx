...
export default function FooterGlow() {
  return (
    <footer className="relative z-10 mt-8 w-full overflow-hidden pt-16 pb-8 bg-white">
      <div className="pointer-events-none absolute top-0 left-1/2 z-0 h-full w-full -translate-x-1/2 select-none">
        <div className="absolute -top-40 left-1/4 h-[600px] w-[600px] rounded-full bg-[#2563EB]/50 blur-[140px]"></div>
        <div className="absolute right-1/4 -bottom-40 h-[600px] w-[600px] rounded-full bg-[#2563EB]/50 blur-[140px]"></div>
      </div>
      <div className="glass relative mx-auto flex max-w-6xl flex-col items-center gap-8 rounded-2xl px-6 py-10 md:flex-row md:items-start md:justify-between md:gap-12 shadow-sm">
        <div className="flex flex-col items-center md:items-start">
          ...
          <p className="text-[#64748B] mb-6 max-w-xs text-center text-sm md:text-left">
            Empowering small businesses with the energy of the next generation. Connect with top student talent fast, locally, and reliably.
          </p>
        ...
        <nav className="flex w-full flex-col gap-9 text-center md:w-auto md:flex-row md:justify-end md:text-left">
          <div>
            <div className="mb-3 text-xs font-semibold tracking-widest text-[#0B1C30] uppercase">
              Product
            </div>
      ...
      <div className="text-[#94a3b8] relative z-10 mt-10 text-center text-xs">
        <span suppressHydrationWarning>&copy; {new Date().getFullYear()} Indian Digital Nomads. All rights reserved.</span>
      </div>
    </footer>
  );
}

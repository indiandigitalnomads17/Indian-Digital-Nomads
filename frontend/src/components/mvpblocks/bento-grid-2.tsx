...
export default function BentoGrid({ items = itemsSample }: BentoGridProps) {
  return (
    <section className="relative overflow-hidden py-12">
      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-4 py-4 md:grid-cols-3">
            ...
            >
              ...
              <CardHeader className="relative space-y-0 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-secondary text-white rounded-md px-2 py-1 text-xs font-medium">
                    {item.status || 'Active'}
                  </span>
                </div>
              </CardHeader>
                ...
                <div className="flex w-full items-center justify-between">
                  <div className="text-muted-foreground flex flex-wrap gap-2 text-xs">
                    {item.tags?.map((tag) => (
                      <span
                        key={`${item.title}-${tag}`}
                        className="bg-secondary text-white rounded-md px-2 py-1 backdrop-blur-xs transition-all duration-200"
                      >
                        #{tag}
                      </span>
                    ))}

...
const styles = sortCx({
    root: "group relative flex max-h-9 w-full cursor-pointer items-center rounded-md bg-transparent outline-focus-ring transition duration-100 ease-linear select-none hover:bg-slate-50 focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2",
    rootSelected: "bg-blue-50/50 hover:bg-blue-50",
});
...
export const NavItemBase = ({ current, type, badge, href, icon: Icon, children, truncate = true, onClick }: NavItemBaseProps) => {
    const iconElement = Icon && (
        <Icon
            aria-hidden="true"
            className={cx(
                "mr-2 size-5 shrink-0 text-slate-400 transition-inherit-all group-hover/item:text-slate-600",
    ...
    const labelElement = (
        <span
            className={cx(
                "flex-1 text-sm font-semibold text-slate-600 transition-inherit-all group-hover/item:text-slate-900",
                truncate && "truncate",
                current && "text-blue-600",
            )}
        >

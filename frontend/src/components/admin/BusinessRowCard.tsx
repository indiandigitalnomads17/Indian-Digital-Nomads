...
import { ShieldTick, Star01, MarkerPin01, Briefcase01, ShoppingBag01 } from '@untitledui/icons';
...
export default function BusinessRowCard({ 
  id, // <-- Destructured the ID parameter smoothly here
  fullName, 
  email, 
  status, 
  nomadScore, 
  ...
  const router = useRouter(); // <-- Initialized client-side navigation router reference context
  ...
  const statusConfig = {
    ACTIVE: { bg: 'bg-emerald-50 text-emerald-700 border-emerald-200', label: 'Active Business' },
    SUSPENDED: { bg: 'bg-amber-50 text-amber-700 border-amber-200', label: 'Suspended' },
    DEACTIVATED: { bg: 'bg-red-50 text-red-700 border-red-200', label: 'Deactivated' }
  }[status] || { bg: 'bg-slate-50 text-slate-600 border-slate-200', label: 'Unknown' };

  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs hover:shadow-md transition-all duration-300 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      
      {/* Left Column: Core Identity Info */}
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            {/* ─── FIXED ROUTING TRIGGER ANCHOR ─── */}
            <h4 
              onClick={() => router.push(`/admin/businesses/${id}`)} 
      ...
      {/* Center Column: Platform Metrics Counts */}
      <div className="flex items-center gap-4 text-xs font-bold text-slate-600 md:mx-auto">
        <div className="bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 flex items-center gap-2">
          <Briefcase01 className="size-4 text-slate-400" />
          <span>{_count?.jobsAsClient || 0} Gigs Posted</span>
        </div>
        <div className="bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 flex items-center gap-2">
          <ShoppingBag01 className="size-4 text-slate-400" />
          <span>{_count?.products || 0} Products</span>
        </div>
      </div>

      {/* Right Column: Nomad Score and Moderation Actions Button */}
        ...
        <button
          ...
          className={`px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all border ${
            status === 'ACTIVE'
              ...
              : 'bg-blue-600 text-white hover:bg-blue-700 border-blue-600 shadow-sm shadow-blue-600/10'
          }`}
        >

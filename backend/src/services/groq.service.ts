      ...
      messages: [
        {
          ...
          1. RIGOROUS SKILL MATCHING: Evaluate the transcript context dynamically. Your absolute priority is to match the technical needs of the job to the existing database skills provided below. Look for frameworks, languages, tools, or operational tasks that map directly to Tier 4 leaf skills.
          2. MAINTAIN ALL ANCESTOR PATHS: If a Leaf Skill (Tier 4, like 'React') matches, you MUST also include its specific Subskill (Tier 3, like 'Frontend Development'), its Parent Skill (Tier 2, like 'Web Development'), and its Category Root (Tier 1, like 'IT Services') nesting them cleanly into each other as shown in the schema.
          ...
          5. Only if a required skill is explicitly vital and completely missing from the tree above should you construct its hierarchy cleanly within the 'suggestedNewSkills' array using appropriate tier tags.
